...
class DependencyExtractor:
    def __init__(
        ...
        previous_latest_csv: Optional[Path] = None,
        previous_release_csv: Optional[Path] = None,
    ):
    ...
    def _get_package_source_url(
        self, dep_name: str, category: str, version: str, source_file: str
    ) -> str:
    ...
    def _is_nvidia_product(
        self, dep_name: str, category: str, source_file: str, notes: str
    ) -> bool:
        ...
        if category == "Base Image" and (
            "pytorch" in dep_name.lower() or "cuda" in dep_name.lower()
        ):
            return True
    ...
    def load_previous_csv(self, csv_path: Path, csv_type: str = "latest") -> None:
        ...
        target_dict = (
            ...
            if csv_type == "latest"
            else self.previous_release_dependencies
        )
        ...
        try:
            ...
            if csv_type == "latest" and len(target_dict) > 0:
                ...
                print(
                    f"Loaded {len(target_dict)} dependencies from previous {csv_type} CSV: {csv_path.name}"
                )
                print(
                    f"Set baseline count to {self.baseline_count} (from previous extraction)"
                )
            else:
                print(
                    f"Loaded {len(target_dict)} dependencies from previous {csv_type} CSV: {csv_path.name}"
        ...
        return {
            "components": {
                ...
                "shared": {
                    ...
                    "requirements": [
                        {"pattern": "container/deps/requirements*.txt", "exclude": []}
                    ],
    ...
    def _format_dependency_name(self, name: str, category: str, version: str) -> str:
        ...
        if category == "Base Image":
            ...
            if "/" in name and "nvidia" in name.lower():
                ...
                parts = name.split("/")
                return self._strip_version_suffixes(
                    self._format_package_name(parts[-1], category)
                )
    ...
    def add_dependency(
        ...
        if self.previous_release_dependencies:
            if key in self.previous_release_dependencies:
                prev_version = self.previous_release_dependencies[key].get(
                    "Version", ""
                )
                if prev_version != version:
        ...
        status = (
            "New"
            if diff_from_latest == "New"
            else ("Changed" if "→" in diff_from_latest else "Unchanged")
        )
        ...
        is_nvidia = self._is_nvidia_product(
            formatted_name, category, source_file, notes
        )
    ...
    def extract_requirements_file(
        self, req_file: Path, component: str, category: str
    ) -> None:
                ...
                if match:
                    ...
                    if operator and version_part:
                        ...
                        if operator == "@":
                            ...
                            version_part = version_part.split(";")[
                                0
                            ].strip()  # Remove markers
                    ...
                    self.add_dependency(
                        ...
                        str(i),
                        f"Python package from {req_file.name}",
                    )
        except Exception as e:
            self.failed_files.append(
                {
                        ...
                        if operator and version_part:
                            if operator == "@":
                                version = (
                                    "from URL"
                                    if (
                                        "git+" in version_part or "http" in version_part
                                    )
                                    else f"@{version_part[:30]}"
                                )
                            else:
                                version = f"{operator}{version_part}"
                        else:
        ...
        try:
            ...
            for match in re.finditer(git_pip_pattern, content):
                ...
                self.add_dependency(
                    ...
                    str(line_num),
                    f"Git-based pip install from {org_name}/{repo_name}",
                )
        except Exception as e:
            self.failed_files.append(
                {
    ...
    def extract_install_script(self, script_path: Path, component: str) -> None:
        ...
        for i, line in enumerate(lines, 1):
            ...
            # Look for pip/uv install commands with version specifiers
            # Pattern: pip install package==version or uv pip install package==version
                ...
                if match:
                    ...
                    if value and value not in ['""', "''", "$2"]:
                        self.add_dependency(
                            ...
                            str(script_path.relative_to(self.repo_root)),
                            str(i),
                            f"From install script: {var_name}",
                        )
    ...
    def extract_all(self) -> None:
        ...
        for component_name, component_config in self.config["components"].items():
            ...
            if k8s_recipes:
                found_recipes = self.discover_requirements_files(
                    k8s_recipes
    ...
    def _extract_all_legacy(self) -> None:
        ...
        for req_file in [
            ...
            if path.exists():
                category = (
                    "Python Package (Test)"
                    if "test" in req_file
                    else "Python Package (Docs)"
                    if "docs" in req_file
                    else "Python Package (Standard)"
                    if "standard" in req_file
                    else "Python Package"
                )
        ...
        self.extract_pyproject_toml(
            self.repo_root / "benchmarks/pyproject.toml", "shared"
        )
    ...
    def write_csv(self, output_path: Path) -> None:
        ...
        if self.previous_latest_dependencies or self.previous_release_dependencies:
            ...
            changed_count = sum(
                1 for d in self.dependencies if d["Status"] == "Changed"
            )
            unchanged_count = sum(
            ...
            if removed:
                ...
                for dep in removed[:10]:  # Show first 10
                    ...
                    print(
                        f"    • {dep['Dependency Name']} (was: {dep['Version']}){critical_flag}"
                    )
    ...
    def print_summary(self) -> None:
        ...
        if unversioned:
            ...
            print("     - Difficulty tracking security vulnerabilities")
            print(
...
def main():
    ...
    parser.add_argument(
        "--github-branch", default="main", help="GitHub branch for URLs (default: main)"
    )
    parser.add_argument(
        "--baseline",
        type=int,
    ...
    if latest_csv is None:
        ...
        if latest_candidate.exists():
            ...
            print(
                f"Auto-detected latest nightly CSV: {latest_csv.relative_to(repo_root)}"
            )
    ...
    if release_csv is None:
        ...
        if releases_dir.exists():
            release_csvs = sorted(
                releases_dir.glob("dependency_versions_v*.csv"), reverse=True
            )
            if release_csvs:
        ...
        if "components" in extractor.config:
            for component_name, component_config in extractor.config[
                ...
                if dockerfiles:
                    ...
                    if found:
                        print(
                            f"  Dockerfiles: {[str(f.relative_to(repo_root)) for f in found]}"
                        )
                    else:
